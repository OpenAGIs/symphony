defmodule SymphonyElixir.LocalTrackerTest do
  ...
  test "local tracker exposes lease status and unsupported local comment helpers" do
    active_issue = %Issue{
    ...
    assert Local.lease_status(active_issue) == :active
    assert Local.lease_status(expired_issue) == :expired
    assert Local.lease_status(%Issue{}) == :unclaimed

    assert {:error, :local_tracker_workpad_not_supported} =
             Local.ensure_workpad_comment("issue-1", "body")

    assert {:error, :local_tracker_comment_update_not_supported} =
             Local.update_comment("comment-1", "updated")
  end
