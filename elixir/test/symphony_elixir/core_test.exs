defmodule SymphonyElixir.CoreTest do
  ...
  test "agent runner persists worker runtime outputs for upload" do
    ...
    try do
      ...
      events = run_dir |> Path.join("codex-events.jsonl") |> File.read!() |> String.split("\n", trim: true) |> Enum.map(&Jason.decode!/1)

      assert metadata["issue"] == %{"id" => "issue-runtime-output", "identifier" => "MT-OUTPUT"}
      assert metadata["outcome"] == %{"status" => "ok"}
      assert metadata["log_file"] == Path.join([logs_root, "log", "symphony.log"]) |> Path.expand()
      assert Enum.any?(artifacts, &(&1["path"] == "README.md"))
      assert Enum.any?(artifacts, &(&1["path"] == "artifact.txt"))
      assert Enum.any?(events, &(&1["event"] == "session_started" && &1["session_id"] == "thread-output-turn-output"))
      assert Enum.any?(events, &(&1["event"] == "notification" && get_in(&1, ["payload", "method"]) == "codex/event/task_started"))
