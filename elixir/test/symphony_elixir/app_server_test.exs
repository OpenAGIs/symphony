defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server advertises local issue tools when the workflow uses the local tracker" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-local-tools-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "LOCAL-91")
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-local-tools.trace")
      tracker_path = Path.join(test_root, "issues.json")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-local-tools.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
        ...
        case \"$count\" in
          1)
            ...
            ;;
          2)
            printf '%s\\n' '{\"id\":2,\"result\":{\"thread\":{\"id\":\"thread-local-91\"}}}'
            ;;
          3)
            printf '%s\\n' '{\"id\":3,\"result\":{\"turn\":{\"id\":\"turn-local-91\"}}}'
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
            ;;
          *)
      ...
      issue = %Issue{
        id: "issue-local-tools",
        identifier: "LOCAL-91",
        title: "Advertise local issue tools",
        description: "Ensure local workflows do not advertise Linear-only tools",
        state: "In Progress",
        url: "https://example.org/issues/LOCAL-91",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Inspect local tools", issue)
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 payload["id"] == 2 and
                   Enum.map(get_in(payload, ["params", "dynamicTools"]) || [], & &1["name"]) == [
                     "local_issue_list",
                     "local_issue_create",
                     "local_issue_state",
                     "local_issue_comment",
