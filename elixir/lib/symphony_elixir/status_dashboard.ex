defmodule SymphonyElixir.StatusDashboard do
  ...
  def init(opts) do
    ...
    enabled =
      resolve_override(enabled_override, Config.observability_enabled?() and dashboard_enabled?())
    ...
    %{
      ...
      | enabled:
          resolve_override(
            ...
            Config.observability_enabled?() and dashboard_enabled?()
          ),
  ...
  defp format_tracker_link_lines do
    tracker_part =
      case tracker_value() do
        {:ok, value} ->
          colorize(value, @ansi_cyan)
  ...
  defp tracker_value do
    case Config.tracker_kind() do
      "local" ->
        case Config.local_tracker_path() do
          path when is_binary(path) and path != "" -> {:ok, "#{path} (local)"}
          _ -> :error
        end
      ...
      _ ->
        case Config.linear_project_slug() do
          project_slug when is_binary(project_slug) and project_slug != "" ->
            {:ok, linear_project_url(project_slug)}
  ...
  defp format_running_summary(running_entry, running_event_width) do
    ...
    event_label =
      format_cell(summarize_message(running_entry.last_codex_message), running_event_width)
